...
class Template {
	...
	public function viewMitgliedCreate($mitgliedtemplate, $dokument, $data, $gliederungen, $mitgliedschaften, $beitragtimeformats, $mailtemplates, $states, $mitgliederflags, $mitgliedertextfields) {
	...
	public function viewMitgliederFilterAction($action, $filter, $matcher, $result) {
		...
		if ($action instanceof ExportDebitsMitgliederFilterAction) {
			...
			$this->smarty->display("mitgliederexportdebitsbeitragselect.html.tpl");
			return;
		}
		...
		if ($action instanceof CalculateBeitragMitgliederFilterAction) {
			$this->smarty->assign("userlist", $this->parseUsers($result["userlist"]));
			$this->smarty->assign("beitraglist", $this->parseBeitragList($result["beitraglist"]));
	...
	public function viewDokumentTemplate($dokumenttemplate, $link, $title, $options = null) {
		...
		if ($dokumenttemplate instanceof ReadBarcodeDokumentTemplate) {
			...
			$this->smarty->display("dokumentcreate_rechnung.html.tpl");
	...
	public function viewDokumentTransition($transition, $filter, $matcher, $result) {
		...
		if ($transition instanceof ForwardDokumentTransition) {
			if (isset($result["inputNeeded"])) {
				$this->smarty->display("dokumenttransition_forward.html.tpl");
			}
		}
		if ($transition instanceof DynamicMitgliedCreateDokumentTransition) {
			if (isset($result["selectMitgliedTemplate"])) {
				$this->smarty->assign("mitgliedtemplates", $this->parseMitgliedTemplates($result["mitgliedtemplates"]));
				$this->smarty->display("dokumenttransition_dynamicmitgliedcreate.html.tpl");
			}
		}
	}
